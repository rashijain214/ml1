...
dataset_dict={
    'trainset':os.path.join(DATADIR, "MLBlocker_train.csv"),
    'testset':os.path.join(DATADIR, "MLBlocker_test.csv"),
    'gan':os.path.join(DATADIR, "GAN_mutated_MLBlocker.csv"),
    ...
    'custom-gan':os.path.join(DATADIR, "custom_GAN_mutated_mlblocker.csv")
}

def main(program, argv):
    main_parser=argparse.ArgumentParser(description="Run MLBlocker Experiments")
    ...
    main_parser.add_argument("-s", type=str, default="mlblocker", help="Specify model during 'train-gan'", choices=[
        'mlblocker',
        'webgraph',
        'adgraph'
    ])
    ...
    if main_args.p=="feature-eng":
        ...
        if main_args.m.startswith('custom'):
            if main_args.m.endswith('mojo'):
                if not os.path.isdir(os.path.join(MODELDIR, "MLBlocker_custom")):
                    print("No custom mojo file detected.")
                    return
            elif main_args.m.endswith('onnx'):
                if not os.path.isfile(os.path.join(MODELDIR, "MLBlocker_custom.onnx")):
                    print("No custom onnx file detected.")
                    return
        if main_args.d=="custom-gan":
            if not os.path.isfile(os.path.join(DATADIR, "custom_GAN_mutated_mlblocker.csv")):
...
def feature_engineering():
    print("[[Feature engineering process of MLBlocker]]")
...
def model_selection():
    ...
    custom_path=aml.leader.save_mojo(os.path.join(MODELDIR,'MLBlocker_custom'))
    ...
    if aml.leader.algo=="gbm":
        ...
        onnxmltools.utils.save_model(onnx_model, os.path.join(MODELDIR, "MLBlocker_custom.onnx"))
        print("Done")
    return
...
def performance_evaluation(dataset_name, model_name):
    ...
    print("[[Performance evaluation process of MLBlocker using dataset: ",dataset_name, ", model: ",model_name,"]]")
    ...
    if model_name.endswith("onnx"):
        print('Loading Model')
        model = 'MLBlocker.onnx'
        if model_name.startswith('custom'):
            model='MLBlocker_custom.onnx'
        ...
        h2o.no_progress()
        if model_name.startswith('custom'):
            model_path=os.path.join(MODELDIR,"MLBlocker_custom",os.listdir(os.path.join(MODELDIR, "MLBlocker_custom"))[-1])
        else:
            model_path=os.path.join(MODELDIR,"MLBlocker_mojo.zip")
        print("Loading model...")
...
def trainGAN(model):
    ...
    feature_set=''
    if model=='mlblocker':
        feature_set=features['feature_columns_mlblocker']
    elif model=='adgraph':
